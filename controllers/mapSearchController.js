...
const searchMaps = async (req, res, next) => {
  try {
    ...
    const { page, perPage, user, artist, sort, server, searchTerm } = req.query;

    // Call the searchMaps function from mapSearchModel.js
    const maps = await mapSearchModel.searchMaps(page, perPage, user, artist, sort, server, searchTerm);
